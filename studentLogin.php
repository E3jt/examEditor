<html>
    ...
    <form method="post">
        ...
        <span>
            ...
            <a href="login.php">Login nauczyciela</a>
        </span>
    </form>

    <div id="registerd">
        Registered succesfully. You are now able to log in.
    </div>
    ...
    <script>
        ...
        // Wszystkie funkcje tutaj są wizualne
    ...
    <?php
        ...
        if (@$_POST['add']){
            ...
            $check_login = mysqli_query($connect,"SELECT `login`,`password` FROM `studentscredentials` WHERE `login` =  '$login'");  // Wyszukiwanie danych odpowiadającyh tym podanym pzez użytkownika
            ...
            @$fetch_p = $fetch[1];
                                if(!$fetch_l){
                                    ...
                                    if(($pas != null AND ($pas == $fetch_p)) AND ($login != null AND ($login == $fetch_l))){
                                        $getid = mysqli_query($connect,"SELECT `id` FROM `studentscredentials` WHERE `login` =  '$login'");
                                        $id = mysqli_fetch_row($getid);
                                        $_SESSION['stid'] = $id[0];                                                                         // ustawianie sesji mówiących o typie użytkownika oraz id 
                                        $_SESSION['state'] = "true";
